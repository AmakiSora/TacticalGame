...
- **文档**：`rl/README.md` 新增「批量对战与排行榜」章节（用法、断点续跑、评分协议），`rl/RELEASE_NOTES.md` 顶部追加条目。
...
- **种子复现**：同种子产出逐字节一致的地图，非法/越界参数由 `sanitizeRandomOptions` 收敛或 400 拒收；模式固定 `standard`（与 RL 本地基线一致），`rl/env.py` 后续只需在创建请求里加 `"mapId": "random", "random": {...}` 即可接入。
