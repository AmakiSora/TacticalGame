...
- **作废模型归档**：v2.1.4–v2.1.8 四个作废模型的 zip、训练断点与单模型档案移入各自目录的 `deprecated/` 子目录（文件全部保留）。用户可见变化：「+ 添加 AI」下拉不再提供作废模型；round_robin 自动发现 18→14；部署上传排除归档目录。排行榜历史战绩与「作废」状态标签不受影响，详见 `rl/docs/RELEASE_NOTES.md` 顶部条目。
