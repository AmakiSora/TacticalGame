...
- **修复 RL 排行榜页双栏布局被宽表挤爆**：`.two-col` 用 `1.2fr 1fr` 布局，而 `fr` 轨道的自动最小尺寸是内容 min-content——对位矩阵 14+ 列不换行表格（`#h2h-table` min-width 720px，实际 1300px+）把左轨道撑爆，右侧「模型详情」面板被挤成约 40px 宽、标题竖排一字一行，视觉上全部粘连。修复：`stats.css` 的 `.two-col` 改为 `minmax(0, 1.2fr) minmax(0, 1fr)`（0 下限让 `.table-wrap` 的横向滚动接管，统计页同布局一并受益）。顺带两处矩阵专项修复：模型列 `position: sticky; left: 0`（宽矩阵横向滚动时行头常驻可见）；≤720px 窄屏下排除 stats.css 对全部 `.data-table` 的卡片化改造（表头隐藏 + 行拆卡片对矩阵是破坏性的），`#h2h-table` 保持完整表格横向滚动。全量 49 文件 / 430 测试通过。
