...
只保留最近 `RL_SNAPSHOT_KEEP`（默认 20）个；环境每局从最近 8 个快照（按保存时间）里随机选一个当对手。
训练初期目录为空时自动用规则对手；快照损坏/被清理时当局降级为 mixed 规则。
评估环境始终只用规则对手，保证训练期胜率与历史模型可比。
自 v2.3.3 起支持锚点对手 `RL_ANCHOR_MODEL`（常驻候选池的强基准，防策略漂移；续训时默认为被续训的模型自身），
且学习率从 `RL_LEARNING_RATE` 线性衰减到 `RL_LR_END`（默认 3e-4→3e-5）。
...
观测在 5974 维棋盘编码后追加 50 维对手上一回合动作历史（共 6024 维）：
...
环境每次迭代都会改变动作空间或观测语义（v1=512，v2.0.0=38，v2.1=54；v2.3 仍 54 动作但观测扩为 5974 维，v2.5 再扩为 6024 维），但历史模型必须始终可玩：
...
- `rl/env_v24.py` 保存 v2.4.x 时期 env.py 的快照（54 动作 / 5974 维观测），
  `rl/run_model_v24.py` 承载全部 v2.3.x/v2.4.x 模型；它同时作为 v2.5 训练时 5974 维锚点对手的观测降级编码器。
- `src/api/bots.ts` 的路由：38 → `run_model_v200.py`，512 → `run_model_v100.py`；
  54 动作按文件名版本分流：≥ v2.5 → `run_model.py`（当前 6024 维环境），
  v2.3/v2.4 → `run_model_v24.py`（5974 维快照），其余 → `run_model_v22.py`（3922 维快照）。
...
- **评估协议（v2.5.0 验收教训固化）**：确定性策略互打方差极高，单次 8 局无统计意义（同配置复测可在 6:2↔4:4 摆动）；
  ...
  累计 ≥24 局（用 `--stats-file` 跨运行累积）并看分座位报表，惊人比分先复测再下结论。
