...
v2.6 起改为**近期加权**（几何递增权重，对手强度紧跟当前策略）。
...
v2.6 起锚点按 `RL_ANCHOR_PROB`（默认 15%）在自对弈局中固定出场，早期无快照时直接当老师。
...
环境每次迭代都会改变动作空间或观测语义（v1=512，v2.0.0=38，v2.1=54；v2.3 仍 54 动作但观测扩为 5974 维，v2.5 曾扩为 6024 维，v2.6 回退 5974 维），但历史模型必须始终可玩：
...
- `src/api/bots.ts` 的路由：38 → `run_model_v200.py`，512 → `run_model_v100.py`；
  54 动作按文件名版本四代分流：≥ v2.6 → `run_model.py`（当前 5974 维环境），
