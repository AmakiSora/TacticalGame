...
| `rl/models/`、`rl/checkpoints/`、`rl/tb/`、`rl/selfplay/`、`rl/distill/` | 训练产物（模型 zip、断点、TensorBoard 日志、自对弈快照、蒸馏数据），均已 gitignore；作废模型归档在 `rl/models/deprecated/`、`rl/checkpoints/deprecated/`，档案在 `rl/docs/models/deprecated/`（见 `rl/docs/MODELS_NOTES.md`「已作废模型」） |
| `arena/`（仓库根） | AI 竞技场数据：`matches.jsonl`（对战记录，入 git）与 `details/`（每局明细，gitignored、不上服务器） |
...
**对称随机图**（半径 5–10，地形/复活点/据点/回合数/行动点/兵种数值/经济与总部全部随机，
2026-09-10 起默认域拓宽至覆盖全部静态图规则空间：行动点 1–8、补给 20–220、总部 80–240、
据点 2–6 个且收入 0–16，裁决权重/据点类型收入/初始单位数量构成也随机——
详见 `rl/docs/RELEASE_NOTES.md` 当日条目），连通性由服务端硬保证。用于训练泛化能力，防止过拟合固定地图布局：
...
`rl/evaluation/evaluate_cross.py` 让两类参与者在进程内引擎上互打（无需游戏服务器）：
RL 模型（`zip 路径`，每座使用其训练时期的编码与合法动作逻辑）与内置算法 AI
（`algo:<name>`，经 local-worker 的 decide 通道进程内走子，规格来自 `algorithms/registry.mjs`）：
...
常用参数：`--player-a/--player-b` 参与者规格（`--model-a/--model-b` 为等价的模型别名）；
`--games` 对局数；`--stochastic` 按策略采样（默认确定性；对算法无意义）；`--max-rounds`
单局回合上限（超过记平局）；`--verbose` 打印每步动作；`--stats-file <路径>` 把每局结果追加到
JSONL 并输出跨运行累计的分座位统计（同一参与者对与地图的记录自动合并，验收用）；
`--details-dir <目录>`（默认 `arena/details`）与 `--no-details` 控制每局明细落盘。
...
evaluate_cross 默认为每个批次在 `arena/details/<批次>.jsonl` 落盘一行一局的完整明细
（批次名 = seed_prefix-时间戳-pid；每局结束立即追加，跑批中断时已完成对局不丢），
`matches.jsonl` 摘要行新增的 `detailFile` 字段指向所属明细文件。明细记录结构：

| 字段 | 内容 |
|---|---|
| `meta` | 局号、地图与随机图种子、双方参与者名/类型（kind=model/algorithm）/编码器版本或算法名/zip 路径、参数（换座/采样/上限/设备/盐）、耗时、事件流完整性（`eventsIncomplete`/`eventGaps`） |
...
与 `algorithms/registry.mjs` 注册的全部内置算法，两两混合 × 全地图批量对战，
结果累积写入 `arena/matches.jsonl`（算法参与者名字为 `algo_<注册名>`）：

```powershell
# 先小规模冒烟（只跑 default 图、3 个模型、2 个算法、每对 2 局）
rl/.venv/Scripts/python.exe rl/evaluation/round_robin.py --maps default --models v2.7.0,v2.4.0,v2.2.0 --algorithms greedy,threat --games 2
...
- **明细数据**：每个 evaluate_cross 批次在 `arena/details/` 落一个 JSONL
...
- **并行**：`--jobs N` 并发多个 evaluate_cross 子进程（每个独立加载模型，显存/内存有限时保持 1）；
  纯算法批次不加载模型，启动与单局耗时都低一档。
- **评分**：跑完执行 `npm run arena-leaderboard`（已并入 `stats-all`），
  从 JSONL 生成 `public/data/arena-leaderboard.json`，页面 `/arena.html` 展示。
  也可以直接在 `/arena.html` 的「评估控制台」页签（`#console` 直达）里网页启动/监控/停止跑批，
...
- **混池语义**：RL 模型与规则算法在同一池评分，可直接跨类型比较强弱；
  榜单/统计/档案按 `kind`（model/algorithm）区分展示。
