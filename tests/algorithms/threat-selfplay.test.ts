// tests/algorithms/threat-selfplay.test.ts
//
// 合法性回归：threat.decide() 返回的每个动作都必须被引擎接受。
// 线上 runner 遇到动作失败会 break 掉整个回合（algorithms/lib/interfaces.mjs），
// 所以"返回非法动作"不是日志噪音，而是白丢行动点：初版在 20 局里有 15 局出现
// action_limit_reached（攻击/治疗/爆破没有按行动点预算门控）。
...
/** 跑一整局，收集 threat 席位被引擎拒绝的动作。 */
async function playGame(mapId: string, seed: number, threatSeat: string) {
  const game = newGame(`legal_${mapId}_${seed}`, mapId, seed);
  ...
  while (game.phase === 'active' && guard++ < 4000) {
    ...
    if (!owner) break;
    const action = owner === threatSeat
      ? await threat.decide(view(game), utils)
...
describe('threat 自博弈合法性（decide 不得返回引擎拒绝的动作）', () => {
  it.each([
    ['default', 'player_a'],
    ['breach', 'player_b'],
    ['desert', 'player_a'],
  ...
  it('兵力清零时仍会补员，而不是交空回合', async () => {
    const game = newGame('wipe', 'default', 999);
    game.units.length = 0;
    ...
    const action = await threat.decide(view(game), utils);
