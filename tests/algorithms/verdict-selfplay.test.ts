// tests/algorithms/verdict-selfplay.test.ts
//
// 合法性回归：verdict.decide() 返回的每个动作都必须被引擎接受。
...
/** 跑一整局，收集 verdict 席位被引擎拒绝的动作。 */
async function playGame(mapId: string, seed: number, verdictSeat: string) {
  const game = newGame(`legal_verdict_${mapId}_${seed}`, mapId, seed);
  ...
  while (game.phase === 'active' && guard++ < 4000) {
    ...
    if (!owner) break;
    const action = owner === verdictSeat
      ? await verdict.decide(view(game), utils)
...
describe('verdict 自博弈合法性（decide 不得返回引擎拒绝的动作）', () => {
  it.each([
    ['default', 'player_a'],
    ['breach', 'player_b'],
    ['desert', 'player_a'],
  ...
  it('兵力清零时仍会补员，而不是交空回合', async () => {
    const game = newGame('wipe_verdict', 'default', 999);
    game.units.length = 0;
    ...
    const action = await verdict.decide(view(game), utils);
